...
class InfiniteScroll extends SqlBase {
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $form['views_infinite_scroll'] = [
      ...
      'content_wrapper' => [
        '#type' => 'radios',
        '#title' => t('Content wrapper'),
        '#default_value' => $options['content_wrapper'],
        '#options' => [
          'container' => t('Container around rows wrapper - every page will have own rows wrapper'),
          'list' => t('List - just list items will be appended'),
        ],
        '#description' => t('Where should new result be added.'),
      ],
    ];
  }
  ...
  public function validateOptionsForm(&$form, FormStateInterface $form_state) {
    if ($form_state->getValue(['pager_options', 'views_infinite_scroll', 'content_wrapper']) == 'list') {
      if ($this->view->getStyle()->getPluginId() != 'html_list') {
        $form_state->setError($form['views_infinite_scroll']['content_wrapper'], $this->t('List option requires using HTML List style.'));
      }
    }
  }
